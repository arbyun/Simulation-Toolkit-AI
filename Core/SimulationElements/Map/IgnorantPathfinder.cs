...
public class IgnorantPathfinder
{
    ...
    public IgnorantPathfinder(IMap map, ICell[] cellsToIgnore)
    {
        ...
        foreach (var allCell in _map.GetAllCells())
            ...
            if (allCell.IsWalkable || cellsToIgnore.Contains<ICell>(allCell))
            {
    ...
    public RogueSharp.Path ShortestPath(ICell source, ICell destination)
    {
        return TryFindShortestPath(source, destination);
    }

    public RogueSharp.Path TryFindShortestPath(ICell source, ICell destination)
    {
        if (source == null)
        ...
        return steps[0] == null ? null : new RogueSharp.Path(steps);
    }
